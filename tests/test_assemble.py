...
# ---------------------------------------------------------------------------
# selection functions: they mutate a CarSelection and print a confirmation
# ---------------------------------------------------------------------------
...
def test_select_car_type_sets_selection_and_prints(capsys, ans, expected_snippet):
...
def test_select_engine_sets_selection_and_prints(capsys, ans, expected_snippet):
    selection = CarSelection()
...
def test_select_brake_sets_selection_and_prints(capsys, ans, expected_snippet):
    selection = CarSelection()
...
def test_select_steering_sets_selection_and_prints(capsys, ans, expected_snippet):
    selection = CarSelection()
...
def test_run_produced_car_incompatible_parts_does_not_run(capsys):
    run_produced_car(_selection(SEDAN, GM, CONTINENTAL, BOSCH_S))
...
def test_run_produced_car_broken_engine_does_not_move(capsys):
    run_produced_car(_selection(SEDAN, EngineType.BROKEN, MANDO, BOSCH_S))
    out = capsys.readouterr().out
...
def test_run_produced_car_valid_combo_prints_full_spec(capsys):
    run_produced_car(_selection(SEDAN, GM, MANDO, BOSCH_S))
    out = capsys.readouterr().out
...
def test_produced_car_pass_for_compatible_combo(capsys):
    check_produced_car(_selection(SEDAN, GM, MANDO, BOSCH_S))
    assert "PASS" in capsys.readouterr().out


def test_produced_car_fail_sedan_continental(capsys):
    check_produced_car(_selection(SEDAN, GM, CONTINENTAL, BOSCH_S))
    assert "FAIL" in capsys.readouterr().out


def test_produced_car_fail_suv_toyota(capsys):
    check_produced_car(_selection(SUV, TOYOTA, MANDO, BOSCH_S))
    assert "FAIL" in capsys.readouterr().out


def test_produced_car_fail_truck_wia(capsys):
    check_produced_car(_selection(TRUCK, WIA, CONTINENTAL, BOSCH_S))
    assert "FAIL" in capsys.readouterr().out


def test_produced_car_fail_truck_mando(capsys):
    check_produced_car(_selection(TRUCK, GM, MANDO, BOSCH_S))
    assert "FAIL" in capsys.readouterr().out


def test_produced_car_fail_bosch_brake_non_bosch_steering(capsys):
    check_produced_car(_selection(SEDAN, GM, BOSCH_B, MOBIS))
    assert "FAIL" in capsys.readouterr().out
